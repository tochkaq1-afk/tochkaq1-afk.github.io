...
const DEFAULTS = {
  ...
  /* Строка под именем. «Конструктор» и «сайты, боты и системы» красивы,
     но человек после них не понял, что ему предлагают и чем это кончится.
     Здесь сказано ровно это, и всё сказанное проверяется на этой же
     странице: скорость — меткой на витрине, заявка — формой внизу */
  txtHeroSub:'Открываются за секунду, заявки падают вам в телеграм. Минск, работаю сам.',
  ...
  /* Прилагательные заменены числами: «быстрая разработка» — оценка, которую
     даёт себе исполнитель, «8 дней» — обещание, за которое можно спросить.
     Все числа на этой странице проверяются, поэтому их и можно писать */
  txtAdvList:[
    'Сайт за 8 дней :: Не два месяца согласований. Срок называю на первом созвоне и держу.',
    'Скорость 90+ из 100 :: Замеряю перед сдачей тем же тестом Google, что и вы. Это число, а не «быстро».',
    'Заявка за секунду :: Падает вам прямо в телеграм. Без почты, без сторонних сервисов, без спама.',
    'Три экрана из трёх :: Телефон, планшет, монитор — проверяю каждый руками, а не надеюсь на «подстроится».',
    'Ноль чужого кода :: Ни конструктора, ни купленной темы. Потолка возможностей нет, лишнего веса тоже.',
    'Правки без меня :: Панель прямо на сайте: тексты и цены меняете сами, в любое время и бесплатно.'
  ].join('|'),
...
function drawText(){
  const map = {
    ...
    place:cfg.txtPlace, tg:cfg.txtTg, heroSub:cfg.txtHeroSub,
