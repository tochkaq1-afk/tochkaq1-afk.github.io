...
const DEFAULTS = {
  ...
  /* PageSpeed — публичное имя теста Google (PageSpeed Insights), под которым
     его знают заказчики; внутри это тот же Lighthouse, но название узнаваемее.
     Балл по шкале 0–100, отсюда приглушённое «/100» рядом */
  txtMacTagL:'PageSpeed',
  txtMacTagBL:'SEO',
  ...
  txtWorkNote:'Сайта в сети нет — показываю обложкой и рассказом',
  ...
  /* Цены временные — намеренно другие числа, чем на остальных сайтах серии,
     чтобы не повторяться. Заменим на настоящие позже. */
  txtSvcFmt:[
    'Страница :: 460 :: 4 :: ',
    'Сайт :: 1290 :: 8 :: чаще всего',
    'Многостраничный :: 1590 :: 10 :: ',
    'Авторский :: 2380 :: 13 :: ',
    'Редизайн :: 970 :: 6 :: чаще всего',
    'Бот :: 690 :: 5 :: чаще всего',
    'Мини-приложение :: 1050 :: 8 :: ',
    'Бот и приложение :: 1640 :: 11 :: ',
    'Приглашение :: 410 :: 3 :: чаще всего',
    'Сайт события :: 740 :: 5 :: '
  ].join('|'),
  ...
  txtSvcAdds:[
    ...
    'Панель заявок :: 560 :: 4',
    'Телеграм-бот :: 410 :: 3',
    'Магазин с оплатой :: 470 :: 4',
    'Тексты под ключ :: 320 :: 2',
    'Логотип и фирстиль :: 260 :: 2',
    'Фото под сайт :: 180 :: 1',
    /* телеграм */
    'Оплата в боте :: 360 :: 3',
    'Каталог товаров :: 420 :: 3',
    'Рассылки :: 270 :: 2',
    'Панель заказов :: 510 :: 4',
    'Сценарии и тексты :: 210 :: 2',
    'Иконка и обложка :: 170 :: 1',
    /* события */
    'Ответы гостей :: 150 :: 1',
    'Карта и дорога :: 100 :: 1',
    'Фото и музыка :: 130 :: 1',
    'Текст приглашения :: 180 :: 1',
    'Короткая ссылка :: 100 :: 1 :: чаще всего',
    'Версия для печати :: 180 :: 2',
    /* всегда */
    'Поддержка :: 70 :: 0 :: чаще всего',
    'Разбор через месяц :: 160 :: 1'
  ].join('|'),
  ...
  /* Ключи стали сроками: «шаг второй» человек и сам сосчитает, а вот когда
     это кончится — главный вопрос, который держат в голове и не задают */
  txtFlowKeys:'день первый|дни 2–3|дни 4–7|день восьмой',
  /* У каждого шага сказано, что заказчик получает на руки. Без этого
     четыре шага — рассказ о моей работе, а не о его сделке */
  txtFlowList:[
    'Разбор :: Полчаса разговора: зачем сайт, кто на него придёт и что должен сделать. К вечеру присылаю карту экранов — что и в каком порядке будет на странице. На руки: список экранов и точная цена, дальше она не меняется.',
    'Идея экранов :: Каждый экран получает свою мысль и свой приём. Приношу варианты шрифтов, кнопок, курсора и движения — выбираете вы, а не принимаете готовое. На руки: собранный первый экран, по нему уже видно, каким будет сайт.',
    'Сборка :: Пишу вручную: чистый HTML, CSS и JS под задачу. Никаких конструкторов и готовых тем. Ссылку на растущий сайт даю сразу — вы смотрите, как он собирается, и правите по ходу, а не в последний день.',
    'Запуск :: Скорость, телефон, заголовки и карточки для соцсетей. Показываю, как самому править тексты и цены. На руки: домен, исходники и сайт, который может продолжить любой разработчик.'
  ].join('|'),
  ...
  /* Возражения: «вопрос :: ответ». Отвечаю прямо, а не «мы индивидуально
     подходим»: обтекаемый ответ на прямой вопрос читается как «нечего сказать».
     Стоят после цен — там, где эти вопросы и появляются */
  txtObjEyebrow:'о чём обычно спрашивают',
  txtObjTitle:'Четыре вопроса, которые вы не зададите вслух',
  txtObjLead:'Отвечаю здесь, потому что до переписки эти сомнения обычно не доживают.',
  txtObjList:[
    'Дорого. На бирже сделают за двести :: Сделают. За двести вы получите шаблон, в который подставили ваше название, и человека, который пропадёт после оплаты. Разница не в вёрстке — вёрстку сегодня умеют все. Разница в том, что у вас будет свой код без чужих ограничений, панель для правок, замеренная скорость и человек, к которому можно вернуться через полгода.',
    'А если мне не понравится? :: Не понравиться в конце нечему: варианты показываю по ходу. Шрифты, кнопки, движение, первый экран — вы выбираете на второй день, а не смотрите готовое на восьмой. Разговора «я представлял себе совсем другое» просто не случается.',
    'А вы не пропадёте после сдачи? :: Исходники отдаю сразу и целиком — сайт ваш, его может продолжить любой разработчик. Это не угроза мне, а ваша страховка. Поддержка стоит 70 BYN в месяц и означает правки в течение месяца, а не «мы вас записали».',
    'У вас мало работ :: Работ столько, сколько есть, и живые открываются по клику — не скриншотами. Показываю ровно то, что сделал, и нигде не пишу «10 лет опыта». Если для вас решает возраст студии, а не сами сайты, — я не ваш вариант, и это нормально.'
  ].join('|'),
  ...
  txtPrWarStamp:'выдан',
  /* Вариант А: не обещания на словах, а то, что видно и проверяется тут же,
     на самом сайте — рядом метки Lighthouse, открытый код, честные факты
     про 16 работ. Прежний текст (срок/результат/цена/поддержка) был тем же
     набором клише, что у любой студии, только своими словами */
  txtPrList:[
    'Скорость :: у каждой работы свой балл Lighthouse, не одна цифра на все шестнадцать',
    'Код :: открой консоль, там нет чужого фреймворка или конструктора',
    'Честно :: нигде не написано «10 лет опыта», если это неправда',
    'Живое :: все работы открываются по клику, не только скриншоты'
  ].join('|'),

  /* стек: строка = название :: зачем нужно. Каждая строка написана через то,
     что инструмент даёт заказчику, а не через то, что он даёт мне. Звёздочки
     («в портфолио пока нет») сняты: самоедство на визитке ничего не объясняет,
     а сомнение сеет. Механизм звёздочки в коде остался — вдруг понадобится. */
  txtStkEyebrow:'чем я работаю',
  txtStkTitle:'Инструменты и зачем они вам',
  txtStkKey:'зачем это нужно',
  txtStkList:[
    'HTML :: Каркас страницы. Семантика даёт поиску понять, о чём сайт, — без неё вас не найдут.',
    'CSS :: Внешний вид и вся анимация. Без тяжёлых библиотек — поэтому страница открывается сразу.',
    'JavaScript :: Любое поведение пишу сам. Нет потолка «как в теме предусмотрели» — есть только время.',
    'canvas :: Частицы и сцены, которые вёрсткой не собрать: падающие лепестки, знак из пыли, пролёт камеры.',
    'SVG :: Чертежи и значки линиями: чёткие на любом экране и почти ничего не весят.',
    'скролл-анимация :: Сцены, которые разворачиваются по мере прокрутки. Человек не смотрит ролик — он им управляет.',
    'скролл-секвенция :: Видео, разложенное на кадры: камера летит по дому ровно с той скоростью, с какой крутят колесо.',
    'адаптив :: Ровно на телефоне, планшете и мониторе. Проверяю каждый, а не полагаюсь на «оно само подстроится».',
    'панель правок :: Свой редактор прямо на сайте: тексты, цены и цвета правите сами — без меня, без CMS и без риска сломать.',
    'Python :: Боты и серверная часть: приём заявок, обработка данных — всё, что должно работать без вашего участия.',
    'телеграм-боты :: Заявки падают вам в личку за секунду. Без почты, без сторонних сервисов и без спама.',
    'Mini App :: Приложение внутри телеграма: свой экран, своя логика, скачивать ничего не нужно.',
    'формы и заявки :: Заявка проверяется на месте и уходит вам в телеграм. Человеку не нужно вспоминать почту.',
    'метрика :: Цели и события: видно, сколько людей дошли до заявки и на каком экране развернулись остальные.',
    'Lighthouse :: Меряю скорость перед сдачей и держу выше 90. Это не обещание, а число, которое вы проверите сами.',
    'WebP :: Картинки весят в разы меньше при том же виде — страница грузится быстрее, а вы не платите за трафик.',
    'SEO-база :: Заголовки, описания, карточки для соцсетей и карта сайта. Ссылка на сайт красиво выглядит везде.',
    'Git :: История правок: любую версию можно вернуть. Сайт нельзя сломать безвозвратно.',
    'выкладка :: Домен, сертификат, обновления. Правка доезжает до боевого сайта за минуту, а не за неделю.',
    'ffmpeg :: Готовлю видео и раскадровку под сцены — так собран пролёт камеры и вращение очков.',
    'React :: Когда у сайта появляется личный кабинет или панель со своей логикой.',
    'Next.js :: Серверный рендер и отдельная страница под каждый товар: так каталог попадает в поиск.',
    'Three.js :: 3D-сцена прямо на странице: товар, который посетитель крутит сам.'
  ].join('|'),
  ...
  /* Минск и свой код тоже правил: не про то, что «честно, зато без прикрас»,
     а про то, что это даёт — так же, как поправил два соседних факта раньше */
  /* «16 работ» — было «7», сколько реально показано в сетке ниже.
     Если в разделе «работы» не появится ещё 9 карточек, эти два места
     разъедутся, и человек, долиставший до сетки, это заметит */
  txtAboutFacts:[
    'Минск :: на связи в своём часовом поясе',
    '16 работ :: разных задач, разных решений',
    'Свой код :: не упрётся в потолок конструктора',
    'Портфолио :: работает, а не просто выглядит'
  ].join('|'),

  /* Кто я: рассказ рядом с фото. Ни лет опыта, ни числа клиентов, ни агентства
     за спиной — всё, что тут написано, проверяется работами на этой же странице.
     Абзацы через |, слот под фото подписан, пока фото нет */
  txtMePhoto:'Фото появится позже',
  txtMePhotoSub:'портрет · 4:5',
  txtMeLead:'Тимофей. Делаю сайты в Минске — сам, от первой линии до выкладки.',
  txtMeText:[
    'Я не собираю страницы из блоков. Каждый экран начинается с вопроса, что человек должен здесь понять и что сделать дальше, — и только после ответа появляется вёрстка. Поэтому у моих работ нет общего лица: у кинотеатра брутализм с кислотным акцентом, у цветочного — мягкая типографика и каталог, который крутится в 3D, у мотосалона — тяжёлая сетка и гео-блок. Один почерк на всех означал бы, что я подставляю в шаблон чужой бизнес.',
    'Код пишу руками. Не из принципа, а потому что конструктор рано или поздно упирается в потолок: нужную анимацию нельзя сделать, лишний вес нельзя убрать, чужой скрипт нельзя выбросить. У ручной вёрстки потолка нет — есть только время.',
    'Считаю, что сайт обязан работать, а не только выглядеть. Поэтому у каждой работы свой замеренный балл скорости, каждая открывается на телефоне так же, как на мониторе, и ни одна не тянет мегабайты ради красивого фона.',
    'И я показываю варианты по ходу — шрифты, кнопки, движение, заставку. Вы выбираете из десятка, а не принимаете готовое в последний день. Так не бывает разговора «я представлял себе совсем другое».'
  ].join('|'),
...
/* у черновика своя ячейка: иначе настройки твикера тут и на боевом
   перетирали бы друг друга — домен-то один */
const STORE = 'stapel.cfg.draft';
try {
...
function drawText(){
  const map = {
    ...
    workOpen:cfg.txtWorkOpen, workNote:cfg.txtWorkNote,
    ...
    objEyebrow:cfg.txtObjEyebrow, objTitle:cfg.txtObjTitle, objLead:cfg.txtObjLead,
    ...
    mePhoto:cfg.txtMePhoto, mePhotoSub:cfg.txtMePhotoSub, meLead:cfg.txtMeLead,
  ...
  /* ЧЕРНОВИК: приставка во вкладке, чтобы не спутать с боевым сайтом */
  document.title = 'ЧЕРНОВИК · ' + cfg.txtName + ' — ' + cfg.txtRole;
...
const WORKS = [
  ...
  { n:'КИНОНОЧЬ',      c:'сайты компаний', t:'частный кинотеатр',d:'брутализм с кислотным акцентом',    s:'10 секций',        img:'assets/works/cinemanight.webp',  url:'/cinemanight/',
    long:'Антикинотеатр продаёт не сеанс, а вечер: панорамные окна, свой зал, никого лишнего. Значит, сайт обязан этот вечер показать, а не описать. Отсюда брутальная сетка, кислотный акцент и крупные кадры зала во всю ширину — человек листает и примеряет на себя. Бронь в два шага: выбрал повод, оставил телеграм. Заявка падает владельцу в личку за секунду.' },
  { n:'МОТОАРЕНА',     c:'каталоги',       t:'мототехника',     d:'фильтры, гео-блок, галерея',         s:'JSON-каталог',     img:'assets/works/motoarena.webp',    url:'/motoarena/',
    long:'У салона были сотни позиций в таблице и ни одной страницы, куда можно прислать покупателя. Собрал каталог, который читает эту же таблицу: добавили строку — на сайте появился мотоцикл, без меня и без программиста. Фильтры по типу, объёму и цене отсекают лишнее за два касания, а блок «как доехать» отвечает на самый частый звонок в салон — где вы находитесь и есть ли парковка.' },
  { n:'FlowerHome',    c:'магазины',       t:'цветы с доставкой',d:'каталог с 3D-анимацией',            s:'CSS 3D',           img:'assets/works/flowerhome.webp',   url:'/flowerhome/',
    long:'Цветы покупают глазами и на бегу — чаще всего с телефона, за пять минут до выхода из дома. Поэтому каталог открывается сразу, карточка букета крутится в 3D без единой лишней картинки, а корзина живёт в выдвижной панели: не нужно уходить со страницы, чтобы понять, сколько уже набрал. Лепестки, падающие на первом экране, нарисованы кодом — они не весят ничего.' },
  /* ---- лендинги: сайта в сети нет, но рассказать о работе есть что */
  { n:'KLAUS',         c:'сайты компаний', t:'ресторан',        d:'меню, галерея, бронирование',        s:'12 секций',        img:'assets/works/klaus.webp',
    long:'У ресторана всего три задачи: показать зал, показать кухню и принять бронь. Всё остальное на сайте ресторана — лишнее. Двенадцать экранов уложены так, что до кнопки «забронировать стол» человек доходит с любого места, а меню открывается прямо на странице — без скачивания PDF, который на телефоне никто не откроет.' },
  { n:'Метриум',       c:'каталоги',       t:'недвижимость',    d:'пролёт камеры по дому на скролле',   s:'скролл-секвенция', img:'assets/works/metrium.webp',
    long:'Квартиру выбирают по ощущению, а его не передать планировкой. Снял видео пролёта камеры по дому и разложил его на кадры: пока человек крутит колесо, камера летит по этажам — не ролик, который надо включить, а сцена, которой он управляет сам. Раскадровка оказалась легче, чем настоящее 3D: грузится быстрее, работает на любом телефоне и не тянет ни одной библиотеки.' },
  { n:'Nuvelle',       c:'магазины',       t:'магазин одежды',  d:'34 товара, видео в карточках',       s:'каталог, SEO',     img:'assets/works/nuvelle.webp',
    long:'Одежду возвращают, когда в жизни она выглядит не так, как на фото. Поэтому в каждой карточке лежит короткое видео: как вещь садится и как движется. Тридцать четыре позиции разложены по разделам так, чтобы до нужной было не больше двух касаний, а описания написаны под поиск — по ним магазин находят, а не только по названию.' },
  { n:'Ray-Ban Meta',  c:'концепты',       t:'концепт',         d:'прокручиваемая сцена из 275 кадров', s:'canvas, ffmpeg',   img:'assets/works/rayban.webp',
    long:'Взял чужой продукт нарочно: на своём проекте всегда можно занизить планку, на известном — нельзя. Двести семьдесят пять кадров вращения очков, собранных из видео, разворачиваются по мере прокрутки. Здесь я проверял не дизайн, а инженерию: как отдать столько кадров, чтобы страница не встала колом на телефоне.' }
];
...
function buildWorks(){
  ...
  wgrid.innerHTML = WORKS.map((w, i) => `
    <div class="wrow bl" style="--d:${i * 70}ms">
      <i class="bl__ph"></i>
      <div class="bl__c">
        <div class="wcard is-open-able" data-cat="${w.c}"
             role="button" tabindex="0" aria-expanded="false" aria-controls="wp${i}">
          ${w.img
...
const FMT = [
  ...
  { g:0, d:'одно предложение, одно действие',
    why:'Берите, если у вас одна услуга или одна акция и нужно, чтобы человек не выбирал, а нажал. Один экран, вся суть сверху, форма заявки внизу — она уходит прямо вам в телеграм. Это самый дешёвый способ проверить спрос: пойдут заявки — из страницы вырастет сайт, и переделывать с нуля не придётся.',
  ...
  { g:0, d:'услуги, работы, цены, контакты',
    why:'То, что заказывают чаще всего, и по делу. До шести экранов, каждый со своей мыслью: что вы делаете, как это выглядит, сколько стоит, как связаться. Дизайн рисуется под вас, а не подставляется в готовую тему. В комплекте панель, где вы сами правите тексты и цены — без меня, без CMS и без страха что-то сломать.',
    inc:['До шести экранов','Дизайн под вас, не шаблон','Панель правки текстов','Показываю, как править самому','Метрика и цели','Форма заявки в телеграм','Скорость 95+','Домен и выкладка','SEO-база'] },
  { g:0, d:'каталог, разделы, много товаров',
    why:'Когда товаров больше десятка и их надо искать, а не листать. Карточки собираются из вашего файла: добавили строку — на сайте появился товар, без меня и без программиста. Фильтры отсекают лишнее за пару касаний, а у каждой карточки свой адрес и своё описание — так каталог находят в поиске по названиям товаров.',
    inc:['Разделы и каталог','Карточки из файла данных','Фильтры и поиск','Панель правки текстов','Показываю, как править самому','Метрика и цели','Скорость 95+','Домен и выкладка','SEO под карточки'] },
  { g:0, d:'сцена, анимация, свой характер',
    why:'Это когда сайт должен запоминаться и его должны пересылать друзьям. Каждый экран придумывается с нуля: сцена, движение, приём, которого нет больше нигде. Я приношу варианты — шрифты, кнопки, курсор, заставку — и вы выбираете, а не принимаете готовое. Дороже и дольше остальных, зато после него спрашивают, кто вам делал сайт.',
  ...
  { g:0, d:'у вас есть сайт, но он устарел',
    why:'Сайт работает, приносит клиентов, но выглядит на десять лет старше вашего дела. Тексты и смысл остаются вашими — меняются дизайн, скорость и телефонная версия. Ничего не теряется по дороге: адреса страниц сохраняются, и позиции в поиске не падают, как бывает при переезде на новый сайт.',
  ...
  { g:1, d:'принимает заказы и отвечает',
    why:'Когда заказы уже идут в личку и вы устали отвечать одно и то же. Бот отвечает на частые вопросы, принимает заявку по шагам и присылает её вам готовой — с именем, телефоном и тем, что человек выбрал. Работает ночью, в выходные и не забывает записать.',
    inc:['Бот на Python','Команды и сценарии','Заявки и заказы вам в личку','Выкладка и запуск'] },
  { g:1, d:'приложение внутри телеграма',
    why:'Когда одной перепиской не обойтись: нужен экран, где выбирают, считают или смотрят. Открывается прямо в телеграме, скачивать ничего не нужно, вход уже выполнен. Люди не уходят из мессенджера — а значит, доходят до конца чаще, чем на сайте.',
  ...
  { g:1, d:'бот и приложение вместе',
    why:'Полный узел: бот разговаривает и уведомляет, приложение показывает и считает, база у них одна. Заказ, сделанный в приложении, тут же виден в боте и падает вам. Берут, когда телеграм становится основным местом продаж, а не дополнением к сайту.',
  ...
  { g:2, d:'свадьба, юбилей, вечеринка',
    why:'Одна страница вместо бумажных открыток и двадцати переписок. Гость открывает ссылку, видит дату, место, дорогу и дресс-код, нажимает «буду» — и ответ приходит вам в телеграм. Вы перестаёте обзванивать и считать в блокноте.',
  ...
  { g:2, d:'событие, которому мало страницы',
    why:'Когда у события есть программа, участники и своя история: конференция, фестиваль, большой юбилей. Несколько экранов, регистрация с ответами, галерея и карта. После события страница не выбрасывается, а становится архивом — с неё же продают следующее.',
...
const ADD = [
  ...
  /* «Обучение» отсюда убрано: показать, как править сайт самому, — не платная
     опция, а часть сдачи. Строка переехала в состав форматов с панелью правок */
  ['после запуска', 'all', [
  ...
  svcFmts.innerHTML = FMT_GROUPS.map((g, gi) => {
    ...
    return `<div class="fgrp${live ? ' is-on' : ''}${open ? ' is-open' : ''}">
      ...
      <div class="fgrp__body"><div class="fgrp__in">
        ...
        ${/* «Кому это подходит» — про выбранный формат, и только когда выбран
              формат из этого ряда. Человек выбирает не по числу экранов,
              а по описанию себя: даём ему узнать себя в тексте */''}
        ${live && FMT[svcF] && FMT[svcF].why
          ? `<p class="fgrp__why"><b>${rows[svcF] ? rows[svcF].n : ''}</b>${FMT[svcF].why}</p>`
          : ''}
    ...
    return `<div class="acol">
      ...
      ${list.map((info, j) => {
        ...
        return `<button class="aopt neon${svcOn.has(i) ? ' is-on' : ''}" type="button" data-a="${i}"
          ...
          ${r.mark ? `<span class="aopt__mark">${r.mark}</span>` : ''}
...
function buildAbout(){
  ...
  /* абзацы рассказа — отдельными узлами, а не одной строкой в data-txt:
     drawText пишет textContent, и разбивка на абзацы в нём бы не пережила */
  const meBody = document.querySelector('.me__body');
  if (meBody){
    meBody.innerHTML = String(cfg.txtMeText).split('|')
      .map(t => t.trim()).filter(Boolean)
      .map((t, i) => `<p style="--d:${i * 90}ms">${t}</p>`).join('');
  }
...
/* ---------------------------------------------- возражения */
/* Раскрывающиеся строки: вопросов четыре, и все четыре разом — это стена
   текста после и без того плотного экрана цен. Открыт всегда один. */
const objList = document.querySelector('.obj__list');

function buildObj(){
  if (!objList) return;

  objList.innerHTML = String(cfg.txtObjList).split('|').map((r, i) => {
    const p = r.split('::').map(x => x.trim());
    if (!p[0]) return '';
    return `<div class="obj__i rv" style="--d:${180 + i * 70}ms">
      <button class="obj__q" type="button" aria-expanded="false" aria-controls="ob${i}">
        <span class="obj__qt">${p[0]}</span>
        <span class="obj__sign" aria-hidden="true"></span>
      </button>
      <div class="obj__a" id="ob${i}"><div class="obj__a__in"><p>${p[1] || ''}</p></div></div>
    </div>`;
  }).join('');

  if (objList.dataset.wired) return;
  objList.dataset.wired = '1';
  objList.addEventListener('click', e => {
    const q = e.target.closest('.obj__q');
    if (!q) return;
    const item = q.closest('.obj__i');
    const open = !item.classList.contains('is-open');
    objList.querySelectorAll('.obj__i.is-open').forEach(n => {
      n.classList.remove('is-open');
      n.querySelector('.obj__q').setAttribute('aria-expanded', 'false');
    });
...
const PF = window.PF = {
  ...
  runHBtn, heroCtaApply, buildWorks, buildSvc, buildFlow, buildObj, buildAbout, buildContact, fitFootMark,
